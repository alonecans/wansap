...
const vcard = 'BEGIN:VCARD\n'  // Jangan Ganti Kak
            + 'VERSION:3.0\n'  // Jangan Ganti Kak
            + 'FN:NuyFaa\n' //ganti nama mu
            + 'ORG: Pengembang Bot Manca;\n' //Ganti Nama Bot Mu
            + 'TEL;type=CELL;type=VOICE;waid=6283815956151:+62 838-1595-6151\n'  //ganti nomor Mu! 
            + 'END:VCARD'
prefix = '#' // Ganti Aja Terserah Mu
blocked = []   //Jangan Ganti
limitawal = 999999 //Bebas Ganti 
memberlimit = 1
fajri = '❑' //Jangan Di Ganti!!
cr = '**' 
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('NuyFaa','red'),color('X','lime'),color('MancaBot','red'))
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		...
		if (isGroupAdmins) return reply('Admin Grup Mah Bebas:V')
    ...
    if (messagesC.includes('#daftar')){
    client.updatePresence(from, Presence.composing)
    const loli = fs.readFileSync('./mp3/daftar.mp3')
    client.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    }
    if (messagesC.includes('#request')){
    client.updatePresence(from, Presence.composing)
    const loli = fs.readFileSync('./mp3/request.mp3')
    client.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    }
    if (messagesC.includes('#owner')){
    client.updatePresence(from, Presence.composing)
    ...
    if (messagesC.includes('what')){
    client.updatePresence(from, Presence.composing)
    const loli = fs.readFileSync('./mp3/what.mp3')
    client.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    }
    if (messagesC.includes('you')){
    client.updatePresence(from, Presence.composing)
    const loli = fs.readFileSync('./mp3/you.mp3')
    client.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    }
    if (messagesC.includes('dalinda')){
    client.updatePresence(from, Presence.composing)
    const loli = fs.readFileSync('./mp3/dalinda.mp3')
    client.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    }
    if (messagesC.includes('goyang')){
    client.updatePresence(from, Presence.composing)
    const loli = fs.readFileSync('./mp3/goyang.mp3')
    client.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    }
    if (messagesC.includes('sepi')){
    client.updatePresence(from, Presence.composing)
    ...
    client.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
		...
		case 'cuttly':
				...
				case 'googletext':
					...
					if (args.length < 1) return reply(`Contoh : ${prefix}googletext FxSx/gans/abiezz`)
					reply(ind.wait())
					sx = await getBuffer(`https://videfikri.com/api/textmaker/gsuggest/?text1=${fxsx}&text2=${ganz}&text3=${abiez}`)
					client.sendMessage(from, sx, image, {quoted: mek})
					await limitAdd(sender)
				break
		case 'tinyurl':
               client.updatePresence(from, Presence.composing) 
                data = await fetchJson(`https://tobz-api.herokuapp.com/api/tinyurl?url=${args[0]}&apikey=Bidmzz1sJ2L1TKyqaMEU`)
                hasil = `link : ${args[0]}\n\nOutput : ${data.result}`
                reply(hasil)
                await limitAdd(sender)
                break
		case 'shrtco':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				client.updatePresence(from, Presence.composing) 
				data = await fetchJson(`http://lolhuman.herokuapp.com/api/shortlink2?url=${args[0]}&apikey=7f5a6556983b0bf183028c20`)
				hasil = `link : ${args[0]}\n\nOutput : ${data.result}`
				reply(hasil)
				await limitAdd(sender)
				break
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/nsfwblowjob?apikey=Bidmzz1sJ2L1TKyqaMEU', {method: 'get'})
					if (anu.error) return reply(anu.error)
...
quoted: mek, caption: `cogan ganteng neh:v`
  ...
  break
		...
		case 'galaxtext':
				...
				ct = body.slice(10)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/galaxywallpaper?apikey=7f5a6556983b0bf183028c20&text=${ct}`)
				client.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'bplogo':
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  client.sendMessage(from, hehe, image, {quoted: mek, caption: 'Nih Kak'})
	} else {
	...
	break
		...
		case 'text2gif':
				...
				break
		case 'attp':
				if (args.length < 1) return reply(`_Teksnya Mana Boss_\n*Contoh ${prefix}attp Gua Cans No Debat*`)
				attp2 = await getBuffer(`https://api.xteam.xyz/attp?file&text=${body.slice(6)}`)
				client.sendMessage(from, attp2, sticker, {sendEphemeral: true, thumbnail: fs.readFileSync('./src/nuy.jpg', 'base64')})
				break
        case 'trigger':
					if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                                                ...
                                                client.sendMessage(from, nobg, sticker, {quoted: ftoko})
                                                fs.unlinkSync(rano)
                                        })
...
break
		case 'ttp':
                    ...
                    Pin.sendMessage(from, buffer, sticker, {sendEphemeral: true, thumbnail: fs.readFileSync('./src/manca.jpg', 'base64')})
                    break
		case 'thunder':
				// NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'imgmaker':
				...
				ainez = body.slice(10)
				reply(ind.wait())
				anu = await fetchJson(`http://lolhuman.herokuapp.com/api/quotemaker?apikey=7f5a6556983b0bf183028c20&text=${ainez}`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
        ...
        case 'husbu2':
		        ...
		        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih Kak Husbunya'})
			    } catch (e) {
		...
		case 'shota':
         ...
         case 'neko':
            ...
            break
				...
				reply(ind.wait())
				buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/toloserti?apikey=7f5a6556983b0bf183028c20&name=${gatauda}`, {method: 'get'})
            ...
            case 'blackpink':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(ind.wait())
                    if (args.length == 0) return reply(`Example: ${prefix + command} Manca`)
                    txt = args.join(" ")
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/textprome/${command}?apikey=7f5a6556983b0bf183028c20&text=${txt}`)
                    client.sendMessage(from, buffer, image, {caption: 'Nih kak....\n\n*Follow!!* : @thenay_xploit_\n\nJangan Pake Nya Doang!!', quoted: ftoko})
                    break
          case 'logogaming':
             if (!isRegistered) return reply( ind.noregis())
				...
				reply(ind.wait())
				logoo = body.slice(8)
                    buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/logogaming?apikey=7f5a6556983b0bf183028c20&text=${logoo}`)
                    client.sendMessage(from, buffer, image, {caption: 'Nih kak....\n\n*Follow!!* : @thenay_xploit_\n\nJangan Pake Nya Doang!!', quoted: ftoko})
                    break
             case 'robot': 
                   if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
               if (args.length < 1) return reply(`Contoh: ${prefix}robot Manca`)
               reply(ind.wait())
                   anja = body.slice(7)			                  
                   ...
                   client.sendMessage(from, ykk, image, {quoted: mek})
                   break
             case 'marvel':
               if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
               ...
               break
				...
				gatauda = body.slice(8)
				reply(ind.wait())
				buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/meme/darkjoke?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: ftoko})
				await limitAdd(sender)
				break
		case 'randomkpop':
		...
		case 'aesthetic':
				...
				client.sendMessage(from, buffer, image, {quoted: ftoko, caption: 'Nih aestheticnya kak...'})
				await limitAdd(sender)
				break
           case 'joox':
				// NuyFaa
		   ...
		   case 'play':   
				...
				client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: ftoko})
				await limitAdd(sender)
				break
		case 'limit':
				// NuyFaa
		...
		case 'vinta':
                ...
                case 'faktaunik': //FxSx
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/faktaunik?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break		
		case 'quotes':
		...
		case 'covidglobal':    
				...
				client.updatePresence(from, Presence.composing) 
			    tels = body.slice(12)
				asu = await fetchJson(`http://lolhuman.herokuapp.com/api/corona/global?apikey=7f5a6556983b0bf183028c20`)
		...
		case 'ceknamaff': 
				...
				//Hayo Mau Ngapain?
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/freefire/${gatauda}?apikey=7f5a6556983b0bf183028c20`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'ceknamaml': 
				...
				//Hayo Mau Ngapain?
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/mobilelegend/${gatauda}?apikey=7f5a6556983b0bf183028c20`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'namaninja': 
				...
				//Hayo Mau Ngapain?
					gatauda = body.slice(11)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/ninja?apikey=7f5a6556983b0bf183028c20&nama=${gatauda}`)
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'infonomor':
		...
		case 'tampar':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/newsinfo?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
		...
		case 'pinterest':
           ...
           case 'glitchtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`Contoh : ${prefix}glitchtext FxSx/Gans`)
					reply(ind.wait())
           ...
           case 'waifu':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/waifu?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           case 'randombts':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/bts?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           case 'randomexo':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/exo?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           /*case 'blackpink':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
					anu = await fetchJson(http://lolhuman.herokuapp.com/api/random/blackpink?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break*/
           case 'imageislamic':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
					anu = await fetchJson(`https://lindow-api.herokuapp.com/api/wallpaper/muslim?apikey=LindowApi`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           ...
           case 'imagegame':
		     ...
		     case 'fakta':
				...
				case 'katabijak':
			        // NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/katabijak?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
			...
			case 'katailham':
				...
				case 'caklontong':
			        // NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/tebak/caklontong?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
					caklontong = `*${anu.pertanyaan}*`
					setTimeout( () => {
					client.sendMessage(from, '*Jawaban :* '+anu.jawaban+ '\n\n Penjelasan: *'+ anu.keterangan+'*', text, {quoted: mek }) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_10 Detik lagi', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_20 Detik lagi', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_30 Detik lagi', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
                ...
                case 'babi':
					...
					const es = groupMembers
					const beb = nge[Math.floor(Math.random() * nge.length)]
					const an = pushname2[Math.floor(Math.random() * es.length)]
					teks = `*Yang Paling Babi Disini Adalah :* @${beb.jid.split('@')[0]} Ngok Ngok`
					membr.push(beb.jid)
				...
				case 'tebakgambar':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/tebak/gambar?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
		...
		case 'spamcall':
			...
			break  
               ...
               client.sendMessage(from, asupan, video, { mimetype: 'video/mp4', quoted: ftoko, caption: 'Nih kak asupan nya' })
               await limitAdd(sender)
               break
               case 'ytmp4':
  if (!isRegistered) return reply( ind.noregis())
...
mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: ftoko
  })
  await limitAdd(sender)
  } catch {
    reply(mess.ferr)
...
mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: ftoko
  })
  await limitAdd(sender)
  } catch {
    reply(mess.ferr)
  ...
  break
       ...
       case 'bahasapurba': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
					purba = body.slice(7)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/bahasapurba?apikey=7f5a6556983b0bf183028c20&text=${purba}`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break
		case 'halah': 
		...
		case 'alay2': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/upperlower?apikey=7f5a6556983b0bf183028c20&text=${body.slice(6)}`)
					reply(data.result)
				    await limitAdd(sender)
					break
		case 'fml':	//FxSx
		...
		case 'cerpen': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/cerpen?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
		...
		case 'quotes': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/quotes?apikey=7f5a6556983b0bf183028c20`, {method: 'get'})
        ...
        case 'stalktwit':
                    ...
                    client.sendMessage(from, anu1, image,{caption: anu2, quoted: ftoko})
                    break
		case 'infofilm': 
					// NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'infofilm2': 
                 if (!isRegistered) return reply( ind.noregis())
					...
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ige = body.slice(10)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/lk21?apikey=7f5a6556983b0bf183028c20&query=${ige}`)
					buffer7 = await getBuffer(anu.result.thumbnail)
		...
		case 'heroml':  
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					 ige = body.slice(9)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/instagram?apikey=7f5a6556983b0bf183028c20&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
				...
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
					 ige = body.slice(12)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/tiktok?apikey=7f5a6556983b0bf183028c20&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'daftar':
                			...
                			if (umurUser < 12) return reply(`your age is too young minimum 12 years`)
                    			...
                    			await client.sendMessage(from, ppimg, image, {quoted: ftoko, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
                    			addATM(sender)
		...
		case 'rate':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					const puuu = sang[Math.floor(Math.random() * sang.length)]
					client.sendMessage(from, 'Pertanyaan : *'+sange+'*\n\nJawaban : '+ puuu+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
                case 'gaycek':
					// NuyFaa
		...
		case 'menu':
              ...
              captionnya = `╭─「 *BOT INFO* 」
${fajri} Nama Bot: Manca
${fajri} Version: 6.4
${fajri} Sc Ori By : FxSx
${fajri} Owner: NuyFaa
╰────────

╭─「 *USER INFO* 」
${fajri} Nama: ${pushname}
${fajri} Uang mu : Rp${uangku}
${fajri} Nomer: ${sender.split("@")[0]}
${fajri} XP: ${getLevelingXp(sender)}/${reqXp}
${fajri} Level: ${getLevelingLevel(sender)}
${fajri} User register : ${_registered.length}
╰────────
...
${fajri} ${prefix}snk
${fajri} ${prefix}lpr 
${fajri} ${prefix}request
${fajri} ${prefix}blocklist
${fajri} ${prefix}banlist
${fajri} ${prefix}blocklist
${fajri} ${prefix}grouplist
${fajri} ${prefix}premiumlist
${fajri} ${prefix}ping
${fajri} ${prefix}buypremium
${fajri} ${prefix}hargaprem
${fajri} ${prefix}menupict
${fajri} ${prefix}menuowner
╰────────

╭─「 *MENU GRUP* 」
${fajri} ${prefix}welcome [1/0]
${fajri} ${prefix}nsfw [1/0]
${fajri} ${prefix}antilink [1/0]
${fajri} ${prefix}leveling [1/0]
${fajri} ${prefix}antibadword [1/0]
${fajri} ${prefix}simih [1/0] 
${fajri} ${prefix}promote [@tag]
${fajri} ${prefix}demote [@tag]
${fajri} ${prefix}tagall
${fajri} ${prefix}listadmin
${fajri} ${prefix}wakillist
${fajri} ${prefix}premiumlist
${fajri} ${prefix}mining
${fajri} ${prefix}hidetag
${fajri} ${prefix}sticktag
${fajri} ${prefix}mctag
${fajri} ${prefix}grouplist
${fajri} ${prefix}add [62]
${fajri} ${prefix}kick [@tag]
${fajri} ${prefix}setname
${fajri} ${prefix}setdesc
${fajri} ${prefix}setpp
${fajri} ${prefix}listadmin
${fajri} ${prefix}linkgc
${fajri} ${prefix}mining
${fajri} ${prefix}level
${fajri} ${prefix}grup [buka/tutup)
╰────────

╭─「 *DOWNLOAD MENU* 」
${fajri} ${prefix}ytmp3 [link]
${fajri} ${prefix}ytmp4 [link]
${fajri} ${prefix}tiktoknowm [link]
${fajri} ${prefix}play [teks]
${fajri} ${prefix}tiktokstalk
${fajri} ${prefix}igstalk
${fajri} ${prefix}igvideo
${fajri} ${prefix}igpost [teks]
${fajri} ${prefix}igtv
${fajri} ${prefix}joox [teks]
╰────────
...
${fajri} ${prefix}neko2
${fajri} ${prefix}waifu
${fajri} ${prefix}animecry
${fajri} ${prefix}animehug
${fajri} ${prefix}osakana
${fajri} ${prefix}naruto
${fajri} ${prefix}anime
${fajri} ${prefix}husbu
${fajri} ${prefix}husbu2
${fajri} ${prefix}animegirl
${fajri} ${prefix}animeboy
${fajri} ${prefix}nekonime
${fajri} ${prefix}mikasa
${fajri} ${prefix}eren
${fajri} ${prefix}titan
${fajri} ${prefix}levi
${fajri} ${prefix}neji
${fajri} ${prefix}jiraya
${fajri} ${prefix}tsunade
${fajri} ${prefix}kiba
${fajri} ${prefix}sarada
${fajri} ${prefix}madara
${fajri} ${prefix}obito
${fajri} ${prefix}tanjiro
${fajri} ${prefix}nezuko
${fajri} ${prefix}luffy
${fajri} ${prefix}zoro
${fajri} ${prefix}sanji
...
${fajri} ${prefix}natsu
${fajri} ${prefix}genos
${fajri} ${prefix}saitama
╰────────

╭─「 *NEW MENU* 」
${fajri} ${prefix}asupan
${fajri} ${prefix}nsfwloli
${fajri} ${prefix}wiki
${fajri} ${prefix}spamcall
${fajri} ${prefix}tebakgambar
${fajri} ${prefix}babi
${fajri} ${prefix}caklontong
${fajri} ${prefix}katailham
${fajri} ${prefix}katabijak
${fajri} ${prefix}fakta
${fajri} ${prefix}mimpi
${fajri} ${prefix}imagemountain
${fajri} ${prefix}loli
${fajri} ${prefix}imagegame
${fajri} ${prefix}imagecyberspace
${fajri} ${prefix}imageislamic
${fajri} ${prefix}randomexo
${fajri} ${prefix}randombts
${fajri} ${prefix}menuownerwaifu
${fajri} ${prefix}nulis
${fajri} ${prefix}nulis2
${fajri} ${prefix}nulis3
${fajri} ${prefix}nulis4
${fajri} ${prefix}sticker
${fajri} ${prefix}stickergif
${fajri} ${prefix}toimg
${fajri} ${prefix}hartatata
${fajri} ${prefix}hartatata2
${fajri} ${prefix}hartatata3
${fajri} ${prefix}text2gif
${fajri} ${prefix}attp
${fajri} ${prefix}trigger
${fajri} ${prefix}wasted
${fajri} ${prefix}pelangi
${fajri} ${prefix}ttp
${fajri} ${prefix}tolol
${fajri} ${prefix}quotemaker
${fajri} ${prefix}hilih [teks]
${fajri} ${prefix}alay [teks]
${fajri} ${prefix}say [teks]
${fajri} ${prefix}wiki [teks]
${fajri} ${prefix}kbbi [teks]
${fajri} ${prefix}map [teks]
${fajri} ${prefix}kalkulator
${fajri} ${prefix}lirik [teks]
${fajri} ${prefix}brainly [teks]l
${fajri} ${prefix}bitly
${fajri} ${prefix}cuttly
${fajri} ${prefix}meme
${fajri} ${prefix}darkjoke
${fajri} ${prefix}tinyurl
${fajri} ${prefix}shrtco
${fajri} ${prefix}chord [teks]
${fajri} ${prefix}katabijak 
${fajri} ${prefix}faktaunik 
${fajri} ${prefix}coronainfo
${fajri} ${prefix}holoh
${fajri} ${prefix}bahasapurba
${fajri} ${prefix}halah
${fajri} ${prefix}huluh
${fajri} ${prefix}heleh
${fajri} ${prefix}hurufterbalik
${fajri} ${prefix}lirik
${fajri} ${prefix}alay2
${fajri} ${prefix}fml
${fajri} ${prefix}katadilan
${fajri} ${prefix}katabucin
${fajri} ${prefix}katabucin2
${fajri} ${prefix}cerpen
${fajri} ${prefix}quotes
${fajri} ${prefix}bacotanhacker
${fajri} ${prefix}quotedoraemon
${fajri} ${prefix}infofilm
${fajri} ${prefix}pantun
${fajri} ${prefix}artinama [teks]
${fajri} ${prefix}bahasa
${fajri} ${prefix}kodenegara
${fajri} ${prefix}infogempa
${fajri} ${prefix}infocuaca
${fajri} ${prefix}infonomor
${fajri} ${prefix}covidindo
${fajri} ${prefix}covidglobal
${fajri} ${prefix}ceknamaff
${fajri} ${prefix}namaninja
${fajri} ${prefix}vinta
${fajri} ${prefix}ccgenerator
${fajri} ${prefix}shrtco
${fajri} ${prefix}newsinfo
${fajri} ${prefix}tribunews
${fajri} ${prefix}nickff
${fajri} ${prefix}quran
${fajri} ${prefix}foxnews
${fajri} ${prefix}liputan6
${fajri} ${prefix}jadwalbola
${fajri} ${prefix}film2
${fajri} ${prefix}film
${fajri} ${prefix}newsinfo
${fajri} ${prefix}coronainfo
...
${fajri} ${prefix}kurs
${fajri} ${prefix}cersex
${fajri} ${prefix}ceritahorror
${fajri} ${prefix}infofilm2
${fajri} ${prefix}heroml
${fajri} ${prefix}newsinfo
${fajri} ${prefix}ceknamaml
${fajri} ${prefix}gantengcek
${fajri} ${prefix}cantikcek
${fajri} ${prefix}sangecek
${fajri} ${prefix}gaycek
${fajri} ${prefix}lesbicek
${fajri} ${prefix}cecan
${fajri} ${prefix}cogan
${fajri} ${prefix}watak
${fajri} ${prefix}hobby
${fajri} ${prefix}apakah
${fajri} ${prefix}kapankah
${fajri} ${prefix}bisakah
${fajri} ${prefix}bagaimanakah
${fajri} ${prefix}rate
${fajri} ${prefix}1cak
${fajri} ${prefix}img [teks]
${fajri} ${prefix}mutual
${fajri} ${prefix}next
${fajri} ${prefix}fototiktok
${fajri} ${prefix}randomkpop
${fajri} ${prefix}randombts
${fajri} ${prefix}randomexo
${fajri} ${prefix}doraemon
${fajri} ${prefix}pokemon
${fajri} ${prefix}anjing
${fajri} ${prefix}kucing
${fajri} ${prefix}hamster
${fajri} ${prefix}kelinci
${fajri} ${prefix}mobil
${fajri} ${prefix}motor
${fajri} ${prefix}sepeda
${fajri} ${prefix}aesthetic
${fajri} ${prefix}bluesky
${fajri} ${prefix}flower
${fajri} ${prefix}icecream
${fajri} ${prefix}pemandangan
${fajri} ${prefix}quotes
${fajri} ${prefix}quoteskehidupan
${fajri} ${prefix}quotesislami
${fajri} ${prefix}katabijak
${fajri} ${prefix}katailham
${fajri} ${prefix}quotesnasehat
${fajri} ${prefix}quotesmotivasi
${fajri} ${prefix}animequotes
${fajri} ${prefix}limit
${fajri} ${prefix}buylimit
${fajri} ${prefix}buypremiumlimit
${fajri} ${prefix}transfer
${fajri} ${prefix}leaderboard
${fajri} ${prefix}quotes
${fajri} ${prefix}gtts
${fajri} ${prefix}kusonime 
${fajri} ${prefix}neonime
${fajri} ${prefix}wait
${fajri} ${prefix}apkpure [teks]
${fajri} ${prefix}happymod [teks]
${fajri} ${prefix}moddroid [teks]
${fajri} ${prefix}blowjob
${fajri} ${prefix}sfw
${fajri} ${prefix}nsfw
${fajri} ${prefix}randomsfw
${fajri} ${prefix}randomnsfw
${fajri} ${prefix}nsfwneko
${fajri} ${prefix}truth
${fajri} ${prefix}dare
${fajri} ${prefix}slap
${fajri} ${prefix}tampar
...
${fajri} ${prefix}wame
${fajri} ${prefix}qrcode
${fajri} ${prefix}profile
${fajri} ${prefix}addstic
${fajri} ${prefix}slist
${fajri} ${prefix}gets
${fajri} ${prefix}addvn
${fajri} ${prefix}vnlist
${fajri} ${prefix}getvn
${fajri} ${prefix}addimg
${fajri} ${prefix}imglist
${fajri} ${prefix}gimg
${fajri} ${prefix}addvid
${fajri} ${prefix}vidlist
${fajri} ${prefix}getvid
╰───────

╭─「 *MAKER MENU* 」
${fajri} ${prefix}darkneon
${fajri} ${prefix}candlemug
${fajri} ${prefix}lovemsg
${fajri} ${prefix}mugflower
${fajri} ${prefix}narutobanner
${fajri} ${prefix}paperonglass
${fajri} ${prefix}romance
${fajri} ${prefix}shadow
${fajri} ${prefix}coffeecup
${fajri} ${prefix}coffeecup2
${fajri} ${prefix}glowingneon
${fajri} ${prefix}underwater
${fajri} ${prefix}hpotter
${fajri} ${prefix}woodblock
${fajri} ${prefix}shadow2
${fajri} ${prefix}cup
${fajri} ${prefix}cup2
${fajri} ${prefix}romance2
${fajri} ${prefix}smoke
${fajri} ${prefix}burnpaper
${fajri} ${prefix}lovemessage
${fajri} ${prefix}undergrass
${fajri} ${prefix}love
${fajri} ${prefix}coffe
${fajri} ${prefix}woodheart
${fajri} ${prefix}flowerheart
${fajri} ${prefix}woodenboard
${fajri} ${prefix}summer3d
${fajri} ${prefix}wolfmetal
${fajri} ${prefix}nature3d
${fajri} ${prefix}underwater
${fajri} ${prefix}golderrose
${fajri} ${prefix}summernature
${fajri} ${prefix}letterleaves
${fajri} ${prefix}glowingneon
${fajri} ${prefix}falleaves
${fajri} ${prefix}flamming
${fajri} ${prefix}potter
${fajri} ${prefix}carvedwood
${fajri} ${prefix}wetglass
${fajri} ${prefix}multicolor3d
${fajri} ${prefix}watercolor
${fajri} ${prefix}luxurygold
${fajri} ${prefix}galaxywallpaper
${fajri} ${prefix}lighttext
${fajri} ${prefix}beautifulflower
${fajri} ${prefix}puppycute
${fajri} ${prefix}royaltext
${fajri} ${prefix}heartshaped
${fajri} ${prefix}birthdaycake
${fajri} ${prefix}galaxystyle
${fajri} ${prefix}hologram3d
${fajri} ${prefix}greenneon
${fajri} ${prefix}glossychrome
${fajri} ${prefix}greenbush
${fajri} ${prefix}metallogo
${fajri} ${prefix}noeltext
${fajri} ${prefix}glittergold
${fajri} ${prefix}textcake
${fajri} ${prefix}starsnight
${fajri} ${prefix}wooden3d
${fajri} ${prefix}textbyname
${fajri} ${prefix}writegalacy
${fajri} ${prefix}galaxybat
${fajri} ${prefix}snow3d
${fajri} ${prefix}birthdayday
${fajri} ${prefix}gplybutton
${fajri} ${prefix}splybutton
${fajri} ${prefix}epep
${fajri} ${prefix}galaxtext
${fajri} ${prefix}thunder
${fajri} ${prefix}imgmaker
${fajri} ${prefix}calendermaker
${fajri} ${prefix}pantaimalam
${fajri} ${prefix}costumwp
${fajri} ${prefix}facebookpage
${fajri} ${prefix}gtav
${fajri} ${prefix}wanted
${fajri} ${prefix}bplogo
${fajri} ${prefix}pencil
${fajri} ${prefix}crossgun
${fajri} ${prefix}bakar
${fajri} ${prefix}blackpink
${fajri} ${prefix}neon
${fajri} ${prefix}greenneon
${fajri} ${prefix}advanceglow
${fajri} ${prefix}futureneon
${fajri} ${prefix}sandwriting
${fajri} ${prefix}sandsummer
${fajri} ${prefix}sandengraved
${fajri} ${prefix}metaldark
${fajri} ${prefix}neonlight
${fajri} ${prefix}holographic
${fajri} ${prefix}text1917
${fajri} ${prefix}minion
${fajri} ${prefix}deluxesilver
${fajri} ${prefix}newyearcard
${fajri} ${prefix}bloodfrosted
${fajri} ${prefix}halloween
${fajri} ${prefix}jokerlogo
${fajri} ${prefix}fireworksparkle
${fajri} ${prefix}natureleaves
${fajri} ${prefix}bokeh
${fajri} ${prefix}toxic
${fajri} ${prefix}strawberry
${fajri} ${prefix}box3d
${fajri} ${prefix}roadwarning
${fajri} ${prefix}breakwall
${fajri} ${prefix}icecold
${fajri} ${prefix}luxury
${fajri} ${prefix}cloud
${fajri} ${prefix}summersand
${fajri} ${prefix}horrorblood
${fajri} ${prefix}thunder2
${fajri} ${prefix}robot
${fajri} ${prefix}marvel
╰────────

╭─「 *MENU OWNER* 」
${fajri} ${prefix}setreply
${fajri} ${prefix}setppbot
${fajri} ${prefix}addbadword
${fajri} ${prefix}delbadword
${fajri} ${prefix}listbadword
${fajri} ${prefix}ban
${fajri} ${prefix}listban
${fajri} ${prefix}unban
${fajri} ${prefix}block
${fajri} ${prefix}unblock
${fajri} ${prefix}setprefix
${fajri} ${prefix}stnuy
${fajri} ${prefix}outmanca
${fajri} ${prefix}join
${fajri} ${prefix}event [1/0]
${fajri} ${prefix}bc
${fajri} ${prefix}bcgc
${fajri} ${prefix}clone
${fajri} ${prefix}clearall
╰────────
...
client.sendMessage(from, menu, image, {quoted: ftoko, caption: captionnya})
		    break
		case 'donasi':
		case 'donate':
					// NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					client.sendMessage(from, donasi(), text, {sendEphemeral: true, thumbnail: fs.readFileSync('./src/manca.jpg', 'base64')})
					break
		case 'menupicture':
		case 'menupict':
		case 'pictmenu':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					wew = fs.readFileSync(`./src/logo2.jpg`)
		...
		case 'bahasa':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					//Hayo Mau Ngapain?
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.sendMessage(from, bahasa(), text, {sendEphemeral: true, thumbnail: fs.readFileSync('./src/manca.jpg', 'base64')})
					await limitAdd(sender)
					break
		case 'kodenegara':
					// NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.sendMessage(from, negara(), text, {sendEphemeral: true, thumbnail: fs.readFileSync('./src/manca.jpg', 'base64')})
					await limitAdd(sender)
					break
		case 'del':
		case 'd':
		...
		case 'blocklist': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, options, text, {sendEphemeral: true, thumbnail: fs.readFileSync('./src/logo3.jpg', 'base64')})
					await limitAdd(sender)
					break
               ...
               case 'profile':
					...
					client.sendMessage(from, buffer, image, {quoted: ftoko, caption: profile})
					break
                case 'quotemaker':
					// NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'dare':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, tod, image, { quoted: ftoko, caption: '*Dare*\n\n'+ der })
					await limitAdd(sender)
					break
		case 'quoteskehidupan':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, crot, image, { quoted: ftoko, caption: '*Quotes Kehidupan*\n\n'+ quo })
					await limitAdd(sender)
					break
		case 'quotesislami':
					// NuyFaa
		...
		/*case 'loli':
                ...
                case 'loli':
					...
					client.sendMessage(from, nye, image, { caption: '*Loli*', quoted: ftoko })
					await limitAdd(sender)
					break
		case 'waifu':
					// NuyFaa
		...
		case 'pokemon':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					pok = await getBuffer(nimek)
					client.sendMessage(from, pok, image, { quoted: ftoko })
					await limitAdd(sender)
					break
				case 'freefireimg':
					// NuyFaa
		...
		case 'kelinci':
				...
				if (!isNsfw) return reply(ind.nsfwoff())
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/kiss?apikey=Bidmzz1sJ2L1TKyqaMEU', {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(ind.stikga())
		...
		case 'osakana':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					reply(ind.wait())
					nya = JSON.parse(JSON.stringify(data));
					nimek =  nya[Math.floor(Math.random() * nya.length)];
					pok = await getBuffer(nimek)
					client.sendMessage(from, pok, image, { quoted: ftoko })
					await limitAdd(sender)
					break
		...
		case 'animeboy':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				    ...
				    client.sendMessage(from, tollp, image, {quoted: ftoko})
				    break
		case 'quotesid':
					// NuyFaa
                 if (!isRegistered) return reply( ind.noregis())
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								client.sendMessage(from, buffer, sticker, {quoted: mek, sendEphemeral: true, thumbnail: fs.readFileSync('./src/manca.jpg', 'base64')})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
		...
		case 'stnuy':
					...
					if (!isOwner) return reply(ind.ownerb())
					fajri = args[0]
					reply(`*setnuy berhasil di ubah menjadi* : ${fajri}`)
					break 
		case 'setlimit':
		case 'addlimit':
					if (args.length < 1) return
	    ...
	    case 'join':
            ...
            faa = args[0]
            if (args.length < 1) return client.sendMessage(from, 'Masukan Link Grup Nya!', text, {quoted: mek})
            var codeInvite = faa.split('https://chat.whatsapp.com/')[1]
		...
		case 'tiktokstalk':
				...
				try {
						...
						client.sendMessage(from, buffer, image, {quoted: ftoko, caption: teks})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
        ...
        case 'linkgroup':
				...
				neh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
			    client.sendMessage(from, neh, text, {sendEphemeral: true, thumbnail: fs.readFileSync('./src/logo2.jpg', 'base64')})
			    break
		case 'hargaprem':
		case 'hargapremium':
					...
					break
		case 'buypremium':
					client.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/6283815956151',MessageType.text, { quoted: mek} )
					break
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST MANCA 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
		...
		case 'group':
            ...
            case 'creator':
            case 'developer':
                  client.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: ftoko})
				  break    
           /*case 'setname':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
					...
					break
                 case 'simi':
                    if (isBanned) return reply('Maaf Kamu Sudah Terban')
					if (args.length < 1) return reply(`Hai ${pushname}`)
					teks = body.slice(4)
					try { 
					anu = await fetchJson(`https://simsumi.herokuapp.com/api?text=Bidmzz1sJ2L1TKyqaMEU`, {method: 'get'})
